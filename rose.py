...
_URL = "https://huggingface.co/datasets/Salesforce/rose/resolve/main/rose_data.tar.gz"  # We may need to change the URL
        ...
        for system in self.systems:
            ...
            if protocol:
                if "gpt" in self.config.name:
                    sys_dict[system] = {"prior": datasets.Value("float64"), "ref_based": datasets.Value("float64"), \
                            "ref_free": datasets.Value("float64"),  "acu_labels": \
                            datasets.features.Sequence(datasets.Value("float64")), \
                            "acu": datasets.Value("float64"), "normalized_acu": datasets.Value("float64")}
                else:
                    sys_dict[system] = {"prior": datasets.Value("float64"), "ref_based": datasets.Value("float64"), \
                            "ref_free": datasets.Value("float64"), "acu_labels": \
                            datasets.features.Sequence(datasets.Value("float64")), \
                            "acu": datasets.Value("float64"), \
                            "normalized_acu": datasets.Value("float64")}
            else:
                sys_dict[system] = {"acu_labels": datasets.features.Sequence(datasets.Value("float64")), \
                        "acu": datasets.Value("float64"), "normalized_acu": datasets.Value("float64")}
